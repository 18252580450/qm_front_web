require(["js/manage/queryQmPlan", "js/manage/workQmResultHistory", "jquery", 'util', "transfer", "easyui", "dateUtil", "commonAjax", "ztree-exedit"], function (QueryQmPlan, QueryQmHistory, $, Util, Transfer, easyui, dateUtil, CommonAjax) {
    //初始化方法
    initialize();
    var userInfo,
        departmentId,   //虚拟组id
        ...
        reqParams = null,
        isCheckParent = false,//设置父节点是否可被选 true 可选 false不可选 默认可选
        isChoice = false,//节点是否区分可选标志 true区分 false不区分 默认不区分(节点是否可被选)
        isVisual = true, // 节点是否区分可见性标志 true区分 false不区分 默认不区分
    ...
    function initPageInfo() {
        ...
        $("#queryInfo").datagrid({
            columns: [[
                ...
                {
                    field: 'srvReqstTypeFullNm', title: '服务请求类型', align: 'center', width: '10%',
                    formatter: function (value, row, index) {
                        if (value) {
                            return "<span title='" + value + "'>" + value + "</span>";
                        }
