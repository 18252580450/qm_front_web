    ...
    function (underscore, tpl, QryCheckTemplate, QryStrategy, $, CommonAjax, Util, Transfer, crossAPI, dateUtil) {
        ...
        function initSearchForm(planBean) {
            ...
            $("#manOrAuto", $el).combobox({
                url: '../../data/distribution_type.json',
                method: "GET",
                ...
                onLoadSuccess: function () {
                    ...
                    if (!planBean) {
                        manOrAuto.combobox('select', data[0].codeValue);
                    }
                }
            });
            $('#planRuntype', $el).combobox({
                url: '../../data/plan_run_type.json',
                method: "GET",
                valueField: 'codeValue',
                textField: 'codeName',
                panelHeight: 'auto',
                editable: false,
                onLoadSuccess: function () {
                    var planRuntype = $('#planRuntype', $el);
                    var data = planRuntype.combobox('getData');
                    if (!planBean) {
                        planRuntype.combobox('select', data[0].codeValue);
                    }
                }
            });
                ...
                if (datas) {
                    ...
                    $('#planType', $el).combobox({
                        ...
                        onSelect: function (value) {
                            ...
                            if (planTypeFlag === "0") {
                                $("#manOrAuto", $el).combobox('enable');
                            } else {
                                $("#manOrAuto", $el).combobox('disable');
                                $("#manOrAuto", $el).combobox('setValue', '1');  //工单质检默认人工分派
                                manOrAutoFlag = "1";
                            }
                        },
                        onLoadSuccess: function () {
            ...
            if (planBean) {
                ...
                if (planTypeFlag === "0" && manOrAutoFlag === "0") {//语音质检及自动分派才会显示质检人信息
                    $("#showDiv", $el).attr("style", "visibility:visible;");
                } else {
                    $("#showDiv", $el).attr("style", "visibility:hidden;");
                }
                if (planTypeFlag === "1") { //工单质检分派方式默认人工分派
                    $("#manOrAuto", $el).combobox('disable');
                }
        ...
        function addZtreeNodes(list) {
            ...
            list.forEach(function (value, index, array) {
                ...
                var newNode = {
                    pId: "0",
                    checkStaffId: value.checkStaffId,
                    checkStaffName: value.checkStaffName,
                    planId: planIdNew,
                    userType: "0",
                    checkedObjectId: "",
                    checkedObjectName: ""
