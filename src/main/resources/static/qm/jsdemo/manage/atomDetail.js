...
define(["jquery", 'util', "easyui"], function ($, Util) {
    ...
    $(document).ready(function() {
        ...
        var searchData = serilizeUrl(decodeURI(searchURL));
         atomId = searchData.atomId;
        initialize();
        if(typeof(searchData.operate)=="undefined"){
            $("#operate").hide();
        }
    ...
    function serilizeUrl(url) {
     ...
     function initialize () {
      ...
      /*   $("#createSkillConfig").find("input.easyui-textbox").textbox({disabled:true});
         $("#createSkillConfig").combobox("disable");*/
...
/*        $popWindow.on("click", "#cancel", function () {
    ...
    function initPopWindow() {
        ...
        $("#page").empty();
        $popWindow = $("<div></div>").appendTo($("#page"));

        //技能配置表单
        var $search = $([
            "<div class='panel-tool-box cl' >",
            "<div class='fl text-bold' title=''>原子详细信息</div>",
            "</div>",
        ...
        $([
            "<div class='mt-10 test-c' id='operate'>",
