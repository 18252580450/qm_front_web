...
define(["jquery", 'util', "easyui"], function ($, Util) {
    ...
    function initPanelDom(options) {
        var paraTypeCdUrl = Util.constants.CONTEXT + "/kc/common/staticdata/msa/getSysBytypeCd?typeId=" + options.para;
        $("#" + options.id).combobox({
            url: paraTypeCdUrl,
    ...
    function addAtomTypeUnit(options) {
        if (options.atomTypeValue == "2" || options.atomTypeValue == "3") {
            $(['<label class="necessary" for=""><span style="color:red;">*</span>选项值</label>',
                '<input type="text" id="' + options.atomTypeValue + 'input" class="easyui-textbox easyui-validatebox" name="OPTVAL"  style="width:200px" title="选项值之间请用英文逗号隔开(如：张三,李四)">',
    ...
    function initWindowEvent() {
        ...
        $popWindow.on("click", "#global", function () {
            ...
            $.ajax({
                url:  Util.constants.CONTEXT  + urlcu,
                type: typecu,
        ...
        function constructParam(param) {
            ...
            $.each(atomTypeIdArr, function (index, atomTypeId) {
                if (atomTypeId == 2 || atomTypeId == 3) {
                    ...
                    $("#" + atomTypeId + "input").validatebox({required:true});
                    if($("#" + atomTypeId + "input").validatebox("isValid")){
                        flag = false;
                    }

                } else if (atomTypeId == 5) {
                    if ($("#" + atomTypeId + "pannel").combobox('getValue')=="") {
                        alert("单位不能为空");
                        flag = false;
                    }
