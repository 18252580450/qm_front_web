...
require(["jquery", 'util', "transfer", "easyui"], function ($, Util, Transfer) {
    ...
    function initGrid() {
        ...
        $("#page").find("#evaluManage").datagrid({
            columns: [[
                {field: 'WORDID', title: '敏感词Id', hidden: true},
                {field: 'ck', checkbox: true, align: 'center'},
                {field: 'SENSITIVEWORD', title: '敏感词', width: '15%'},
                {field: 'SUBSTITUTEWORD', title: '替换词', width: '15%'},
                {field: 'ARGESEQNO', title: '排列序号', width: '10%'},
                {field: 'CRTTIME', title: '创建时间', width: '10%'},
                {field: 'MODFTIME', title: '修改时间', width: '10%'},
                {field: 'OPPRSNID', title: '操作人员ID', width: '10%'},
                {field: 'TENANTID', title: '租户ID', width: '10%'},
                {field: 'RMK', title: '备注', width: '11%'},
                {
                    field: 'action', title: '操作', width: '6.5%',

                    formatter: function (value, row, index) {
                        var sens = {
                            'wordId': row.WORDID, 'sensitiveWord': row.SENSITIVEWORD,
                            'substituteWord': row.SUBSTITUTEWORD, 'rmk': row.RMK
                        };
                        var sensStr = JSON.stringify(sens);   //转成字符串

                        var Action =
                            "<a href='javascript:void(0);' class='reviseBtn' id =" + sensStr + " >修改</a>";
                        return Action;
                    }
                }
            ]],
            ...
            loader: function (param, success) {
                ...
                Util.ajax.getJson(Util.constants.CONTEXT + "/sensword/querysenswordlist", params, function (result) {
    ...
    function initGlobalEvent() {
        //查询
        $("#searchForm").on("click", "#selectSen", function () {
        ...
        //绑定删除按钮事件
        $("#page").delegate("#delSens", "click", function () {

            var selRows = $("#evaluManage").datagrid("getSelections");
            if (selRows.length == 0) {
            ...
            $.messager.confirm('确认删除弹窗', '确定要删除吗？', function (confirm) {
                ...
                if (confirm) {
                    Util.ajax.deleteJson(Util.constants.CONTEXT + "/sensword/deletesensword", ps, function (result) {
                        ...
                        if (rspCode == "1") {
                            $("#evaluManage").datagrid('reload'); //删除成功后，刷新页面
                        }
                    });
    ...
    function initWindowEvent() {
        ...
        */
        $("#page").on("click", "#addSens", function () {
            // $("#sensitiveWord").textbox({disabled:false}); //文本框只读取消
