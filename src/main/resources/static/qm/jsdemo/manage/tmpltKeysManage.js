...
require(["jquery", "transfer", "loading", 'util', "easyui", 'ztree-exedit', 'js/manage/configAtomAdd'], function ($, Transfer, Loading, Util, EasyUI, Ztree, ConfigAtomAdd) {
    ...
    function getsysCode(codeTypeCd) {
        var result = {};
        $.ajax({
            url:Util.constants.CONTEXT +  "/kc/common/staticdata/msa/getSysBytypeCd?typeId=" + codeTypeCd,
    ...
    function initSearchForm() {
        ...
        var paraTypeCdUrl = Util.constants.CONTEXT + "/kc/common/staticdata/msa/getSysBytypeCd?typeId=" + para_type_cd;
        $("#searchForm").find("[name='paraTypeCd']").combobox({
            url: paraTypeCdUrl,
        ...
        $("#page").delegate("a.deleteBtn", "click", function () {
            ...
            $.messager.confirm('确认删除弹窗', '您确定要删除这条原子记录吗？', function (confirm) {
                if (confirm) {
                    ...
                    Util.ajax.getJson(Util.constants.CONTEXT+"/kc/tmplt/tmpltsvc/msa/tmpltkeyobjrel/isatomused?atomId=" + atomId, null, function (result) {
                        //刷新节点
                        if(result.RSP.RSP_CODE=="1"){
                            ...
                            var atomId = thisDelBtn.attr('id').substr(3);
                            Util.ajax.deleteJson(Util.constants.CONTEXT + "/kc/tmplt/atomsvc/msa/deleteevaluation?atomId=" + atomId, " ", function (result) {
                                //刷新节点
